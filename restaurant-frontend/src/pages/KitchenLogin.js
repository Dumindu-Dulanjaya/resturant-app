...
import './SuperAdminLogin.css'; // Reusing SuperAdminLogin styles
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response?.data?.success) {
                ...
                if (!user || !access_token) throw new Error('Invalid login response from server.');

                const role = user.role?.toString().trim().toLowerCase();
                if (role !== 'kitchen') {
                    Swal.fire({ icon: 'error', title: 'Access Denied', text: 'This login is for Kitchen Staff only.' });
                    return;
                }
                ...
                Swal.fire({
                    ...
                    didClose: () => { window.location.href = '/kitchen/dashboard'; }
                });
            } else {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error?.response?.data?.message || error?.message || 'Failed to login. Please check your credentials.'
            });
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="sa-login-page">
            ...
            <div className="sa-card">
                <h2 className="sa-card-title">Anawuma Login</h2>
